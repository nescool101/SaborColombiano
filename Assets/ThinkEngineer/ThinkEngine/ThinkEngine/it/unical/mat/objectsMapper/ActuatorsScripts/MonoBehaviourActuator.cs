...
namespace ThinkEngine
{
    public  class MonoBehaviourActuator : MonoBehaviour
    {
        ...
        internal void Configure(InstantiationInformation information, string mapping)
        {
            ...
            _mapping = "setOnActuator("+ ASPMapperHelper.AspFormat(features.PropertyAlias) + "(" + ASPMapperHelper.AspFormat(gameObject.name) + ",objectIndex(" + gameObject.GetEntityId() + ")," + mapping + "))";
        ...
        internal string Parse(AnswerSet answerSet)
        {
            ...
            foreach (string literal in answerSet.GetAnswerSet())
            {
                ...
                if (matcher.Success && int.Parse(matcher.Groups[1].Value) == gameObject.GetEntityId())//if the index of the object associated to the actuator is different from the one of the literal
                {
